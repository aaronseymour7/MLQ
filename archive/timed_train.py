...
Changes in this version:
  - Added wall-clock timing: per-iteration duration, running elapsed time,
    an ETA for N_ITERS based on a rolling average, and a final training-time
    summary (also stashed in the saved pickle) so you don't have to eyeball
    terminal timestamps to know how long a run took or how long it has left.
"""
...
def format_hms(seconds):
    """Human-readable H:MM:SS (or M:SS if under an hour)."""
...
if __name__ == "__main__":
    ...
    # -- timing setup --
    # `iter_times` is a short rolling window (not the full history) so the
    # ETA reacts to recent speed (e.g. after JIT warm-up on iter 0, or if
    # chunk_size/behavior changes mid-run) rather than being dragged down
    # by the first few slow iterations forever.
