...
circuit (Qiskit) and evaluates it — keeping the same Hamiltonian, gate basis,
and diagnostic output format as the UCJ script.
...
--------
  ...
  5.  Evaluate energy from statevector + circuit metrics
...
Dependencies
------------
    pip install tenpy qiskit qiskit-aer scipy numpy
"""
...
# =============================================================================
# ENERGY EVALUATION  (pure Qiskit / numpy — no PennyLane)
# =============================================================================
def build_heisenberg_sparse_op(n: int, nn_edges: list, nnn_edges: list,
                                j1: float = J1, j2: float = J2) -> np.ndarray:
    """
    Build the full 2^n × 2^n Heisenberg Hamiltonian as a dense numpy array
    directly from Pauli products, for use with a statevector.

    H = Σ_{<i,j>} J/4 * (XX + YY + ZZ)
    """
    dim = 1 << n
    ...
    def kron_op(op_dict: dict) -> np.ndarray:
        """Tensor product of single-site operators; op_dict maps site → matrix."""
        result = np.array([[1.0 + 0j]], dtype=np.complex128)
        for s in range(n):
            result = np.kron(result, op_dict.get(s, I))
        return result

    for edges, j in [(nn_edges, j1), (nnn_edges, j2)]:
        for si, sj in edges:
            for op in [X, Y, Z]:
                H += (j / 4.0) * kron_op({si: op, sj: op})

    return H


def circuit_energy(qc_transpiled: QuantumCircuit,
                   n: int,
                   nn_edges: list,
                   nnn_edges: list,
                   j1: float = J1,
                   j2: float = J2) -> float:
    """
    Evaluate <ψ|H|ψ> from the circuit's statevector.

    Qiskit uses little-endian bit ordering (qubit 0 = LSB); we reverse back
    to big-endian (site 0 = MSB) to match the ED basis before contracting.
    """
    sv_le = Statevector(qc_transpiled).data          # little-endian, shape (2^n,)
    sv_be = _reorder_sv_for_qiskit(sv_le, n)         # → big-endian (same permutation)

    H = build_heisenberg_sparse_op(n, nn_edges, nnn_edges, j1, j2)
    energy = float(np.real(sv_be.conj() @ H @ sv_be))
    return energy
...
def write_summary_file(
    ...
    json_data = dict(
        ...
        e_exact=e_exact, e_dmrg=e_dmrg, e_circuit=e_circuit,
...
# MAIN  run()
# =============================================================================
def run(lattice, j1: float = J1, j2: float = J2,
