...
def _optimise_layer_instrumented(
    ...
    opt_x, opt_E, record
    """
    # Block norms for init vector decomposition
    x0_jJ      = x0[:n_pair]
    x0_gK      = x0[n_pair:]
...
def _cold_baseline(
    ...
    """
    Run `n_restarts` cold (random) starts for layer k and log them to tracker.
    Called after the warm restarts so the report can compare both.
    """
...
def adaptive_ucj(variant, n, k_max, e_tol, C_jastrow, e_exact,
    ...
    """
    Adaptive UCJ optimisation with integrated diagnostics.

    All restarts (warm and cold baseline) are logged to `tracker`.
    Call tracker.report() after this returns to see the full breakdown.
    """
    ...
    for k in range(1, k_max + 1):
        ...
        # Build JIT functions once per layer (shared across all restarts)
        ...
        # ── Cold baseline (optional, for comparison) ────────────────
...
def rdm_norms_at_convergence(params: np.ndarray, variant: str, k_layers: int,
    ...
    # Build 1-RDM via <ψ|c†_i c_j|ψ>
    # occupation basis: spin-up = bit-1
    basis_arr = np.array(
        [[(1 if (b >> s) & 1 else -1) for s in range(n)] for b in basis_np],
        dtype=np.float64)
    occ  = (basis_arr + 1) / 2           # shape (dim, n)
    probs = np.abs(psi) ** 2             # shape (dim,)
    ...
    # Off-diagonal: use the same Jordan-Wigner + ratio trick as extract_ucj_correlators
    for i in range(n):
        for j in range(i + 1, n):
            mask = (basis_arr[:, i] == 1) & (basis_arr[:, j] == -1)
            if not mask.any():
    ...
    # Frobenius norms of off-diagonal Re / Im parts
...
def run(n: int, variants=VARIANTS, k_max: int = K_MAX,
    ...
    # Print quick signal summary (independent of tracker)
    ...
    for variant in variants:
        ...
        # One tracker per variant — accumulates all restarts for this run
        tracker = DiagnosticTracker(
        ...
        # Print per-variant diagnostic report
        tracker.report()
...
if __name__ == '__main__':
    timer = Timer()
    run(n=24, variants=['re', 'im', 'g'], k_max=K_MAX, e_tol=E_TOL, timer=timer)
